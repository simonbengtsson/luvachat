	...
	return (
		<Tooltip>
			<TooltipTrigger render={<span className={cn(
            						...
            						className,
            					)} {...props} />}>{date.toLocaleTimeString("en-US", format)}</TooltipTrigger>
			<TooltipContent>
...
function ChatMessageAction({
	...
	return (
		<Tooltip>
			<TooltipTrigger render={<Button variant="ghost" size="icon" className={cn("h-7 w-7", className)} {...props} />}>{children}<span className="sr-only">{label}</span></TooltipTrigger>
			<TooltipContent>
				<p>{label}</p>
			</TooltipContent>
		</Tooltip>
